...
void mexFunction(int nhls, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	int dimx_xp, dimx_tp, xist_rna, tsix_rna, xist_prom, xist_rna2, tsix_rna2, xist_prom2, overlap_X_T_1, overlap_X_T_2;
...
//Gillespie reactions
        ...
        elongation (overlap_X_T_1, dimx_xp, xist_gene, tsix_gene, sum_xp, sum_tp, 
            xpol_ini, tpol_ini, xist_rna, tsix_rna, 
            all_xist, temp_sum_xr, temp_sum_tr, temp_sum_xp, temp_sum_tp, dist, step, p[8]);

        //second chromosome
        if (sec_chr==1) {
            elongation (overlap_X_T_2, dimx_xp, xist_gene2, tsix_gene2, sum_xp2, sum_tp2, 
