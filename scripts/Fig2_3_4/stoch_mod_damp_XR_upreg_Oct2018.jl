### Simulate dampening with 'cXR upregulation' scenario
...
ma_scan=Array{Float64}(size(pars,1)*nr_cells,33+2*(Int64(t)+1))
@time for n=1:size(pars,1)
  println(n)
  p=pars[n,:]
  println(p)
  ma_scan[(n-1)*nr_cells+1:n*nr_cells,1:33] = p'.*ones(nr_cells,33)
  ma_scan[(n-1)*nr_cells+1:n*nr_cells,34:33+2*(Int64(t)+1)] = sim_ba_gil_damp_cXR_upreg(p,nr_cells,t)
end
