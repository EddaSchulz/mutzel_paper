...
par_n = lhsu(1*ones(4,1),5*ones(4,1),nr_par);
par_k = 10.^lhsu(-2*ones(4,1),1*ones(4,1),nr_par);

p_all = zeros(nr_par,33);
p_all(:,[3:2:5,11:2:13]) = par_n;
p_all(:,[4:2:6,12:2:14]) = par_k;

%tXA and cXR are present,the other regulator types are absent
p_all(:,26)=1;
p_all(:,27)=1;
...
% write parameter sets and simulation results in file 'tXA_cXR_model_scan_for_stoch_sim.txt'
% Uncomment to construct file
%dlmwrite('../../simulations/Fig2_3_4/tXA_cXR_model_scan_for_stoch_sim.txt',out,'-append')
end
toc